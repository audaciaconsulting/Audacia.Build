...
1. **Initialization**
    - The template checks if we have a working temporary directory established by an earlier “initialize” step.
...
3. **Dynamic Generation of the ZAP Configuration**
    ...
    - This includes context definitions, the site’s base URL, optional authentication details, spider rules, active scanning parameters, and reporting instructions.
...
4. **Publishing & Logging of the Config File**
    - The `zap-automation.yaml` file is stored as a build artifact, so you can later review exactly how the scan was configured.
    - It is also written to the pipeline logs to simplify debugging or troubleshooting.

5. **Executing ZAP in Docker**
    - We run a ZAP Docker container, mounting the working directory.
    - ZAP reads our dynamically generated `zap-automation.yaml` and executes:
...
6. **Renaming Reports**
    - The automation framework outputs “ZAP-Report*.xml” and “ZAP-Report*.html” files. We rename or copy them to `Report.xml` and `Report.html` for consistency.
...
1. **ZAP Automation Config (`zap-automation.yaml`)**
    - Contains all scanning definitions, from spider rules and active scanning policies to reporting instructions.
...
jobs:
  - job: OWASP_Security_Tests
    ...
    steps:
      ...
      # Run the new OWASP ZAP Automation scan template.
